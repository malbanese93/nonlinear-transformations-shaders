...
namespace RuntimeGizmos
{
	...
	public class TransformGizmo : MonoBehaviour
	{
		...
		{
            // NOTE we only need translation in world space!
		...
		void GetTarget()
		{
			if(selectedAxis == Axis.None && Input.GetMouseButtonDown(0))
			{
				...
				{
                    // MODIFIED: INTERACT ONLY WITH LATTICE VERTICES
                    var gameobject = hitInfo.transform.gameObject;
                    target = (gameObject.GetComponent<LatticeVertexScript>() != null) ? gameObject.transform : null;
